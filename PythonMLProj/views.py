...
def predict_ratings_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            temp = classifier.predict_proba(book_info_df_trans)
            book_rating_pred = classifier.predict(book_info_df_trans)

            # Return results to the view to be displayed
            return render(request, 'result-ratings.html', {'num_pages': num_pages, 'text_reviews_count': text_reviews_count, 'publicationYear': publicationYear, 'language_code_grouped': language_code_grouped, 'author_number_books': author_number_books, 'predicted_book_rating': ["prob",temp,"rate",book_rating_pred]})
